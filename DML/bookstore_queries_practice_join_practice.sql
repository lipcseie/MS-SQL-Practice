...
-- This one retrieves the total revenue generated by each customer, sorted from highest to lowest.
...
-- top 5 best-selling books based on the total quantity sold.
SELECT TOP 5 B.BookID, B.Title, SUM(O.Quantity) AS TotalSold
FROM Books B
JOIN  Orders O ON B.BookID = O.BookID
GROUP BY B.BookID, B.Title
ORDER BY TotalSold DESC;
...
--  retrieves the total revenue generated by each book, sorted in descending order:
...
-- top 5 best-selling books based on total quantity sold
SELECT TOP 5 B.BookID, B.Title, 
SUM(O.Quantity) AS TotalSold
FROM Books B
JOIN Orders O ON B.BookID = O.BookID
...
-- retrieves the top 3 customers who have spent the most money on orders:
...
-- retrieves the top 5 best-selling book genres based on the total quantity of books sold:
SELECT TOP 5 B.Genre, 
SUM(O.Quantity) AS TotalBooksSold
FROM Books B
JOIN Orders O ON B.BookID = O.BookID
...
--  retrieves the top 3 customers who have spent the most money on books:
SELECT TOP 3  C.CustomerID, C.FirstName, C.LastName, 
SUM(O.Quantity * B.Price) AS TotalSpent
FROM Customers C
JOIN Orders O ON C.CustomerID = O.CustomerID
...
-- retrieves the top 5 best-selling book genres based on total quantity sold:
SELECT TOP 5 B.Genre, 
SUM(O.Quantity) AS TotalBooksSold
FROM Books B
JOIN Orders O ON B.BookID = O.BookID
...
--  top 5 best-selling book genres based on the total quantity sold:
SELECT TOP 5 B.Genre, 
SUM(O.Quantity) AS TotalBooksSold
FROM Books B
JOIN Orders O ON B.BookID = O.BookID
...
-- Retrieve the Top 5 Customers Who Spent the Most Money
SELECT TOP 5 C.CustomerID, C.FirstName, C.LastName, 
SUM(O.Quantity * B.Price) AS TotalSpent
FROM Customers C
JOIN Orders O ON C.CustomerID = O.CustomerID
...
-- Retrieve the Total Revenue Generated by Each Month of the Current Year
SELECT MONTH(O.OrderDate) AS Month, SUM(O.Quantity * B.Price) AS MonthlyRevenue
FROM Orders O
JOIN Books B ON O.BookID = B.BookID
WHERE YEAR(O.OrderDate) = YEAR(GETDATE()) 
...
SUM(O.Quantity * B.Price) AS MonthlyRevenue
FROM Orders O
JOIN Books B ON O.BookID = B.BookID
WHERE YEAR(O.OrderDate) = YEAR(GETDATE())
GROUP BY MONTH(O.OrderDate)
